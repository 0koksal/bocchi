...
function createWindow(): void {
  ...
  mainWindow.on('ready-to-show', () => {
    mainWindow?.show()
    // Open DevTools with F12 in production too
...
if (gotTheLock) {
  app.whenReady().then(async () => {
    ...
    // Initialize xxhash for WAD parsing
...
function setupIpcHandlers(): void {
  ...
  // Pre-import status
  ipcMain.handle('preimport:get-status', () => {
    return preImportService.getStatus()
  })

  // Pre-import a skin (triggered when user selects a skin in the UI)
  ipcMain.handle('preimport:queue', async (_, skinPath: string, championName: string) => {
    try {
      await preImportService.queueImport(skinPath, championName)
      return { success: true }
    } catch {
  ...
  ipcMain.handle('download-all-skins-bulk', async (event, options) => {
    ...
    try {
      ...
      const tempDir = require('path').join(require('electron').app.getPath('temp'), 'bocchi-bulk-download', 'extracted')
      try {
        const entries = require('fs').readdirSync(tempDir)
        console.log('[BulkDownload] Temp dir contents:', entries)
        event.sender.send('download-all-skins-bulk-progress', {
          phase: 'completed',
          overallProgress: 100,
          currentFile: `DEBUG: temp entries = ${entries.join(', ')}`
        })
      } catch (e) {
        console.log('[BulkDownload] Could not read temp dir:', e)
      }
      return { success: true }
    } catch (error) {
    ...
    try {
      ...
      const skinInfosToProcess = await Promise.allSettled(
        filteredSkins.map(async (skinKey) => {
          ...
          const isClassicSkin = selectedSkinContext?.skinId?.includes('_classic_') || false
          // For classic skins, pass the skinId so URL constructor can extract the classic num
          const skinFileForUrl = isClassicSkin && selectedSkinContext?.skinId ? selectedSkinContext.skinId : skinFile
