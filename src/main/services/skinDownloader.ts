...
export class SkinDownloader {
  ...
  async downloadSkin(url: string): Promise<SkinInfo> {
    ...
    try {
      ...
      // Queue pre-import in background (non-blocking)
      // REMOVED: pre-import now triggers on skin selection, not download
      ...
      if (axios.isAxiosError(error) && error.response?.status === 404) {
        ...
        if (fallbackUrl) {
          ...
          try {
            ...
            const writer = createWriteStream(zipPath)
            await pipeline(fallbackResponse.data, writer)
  ...
  private parseGitHubUrl(url: string): SkinInfo {
    ...
    // Handle classic skin URLs: classic/{jadeChampionId}/{jadeSkinId}/{jadeSkinId}.fantome
    const classicSkinPattern = /^classic\/(\d+)\/(\d+)\/(\d+)\.(zip|fantome)$/
    const classicSkinMatch = relativePath.match(classicSkinPattern)

    if (classicSkinMatch) {
      const jadeChampionId = parseInt(classicSkinMatch[1], 10)
      const jadeSkinId = classicSkinMatch[2]
      const realChampionId = jadeChampionId - 60000

      // Resolve champion name from the real champion ID
      ...
      let skinName = `${jadeSkinId}.fantome`

      if (champion) {
        // Find the classic skin by its num (jadeSkinId % 1000 = classic num, stored with num offset +10000)
        const classicNum = parseInt(jadeSkinId, 10) % 1000
        const classicSkin = champion.skins.find(
          (s) => s.skinType === 'classic' && s.num === classicNum + 10000
        )
        if (classicSkin) {
    ...
    // Handle classic skin chroma URLs: classic/{jadeChampionId}/{parentSkinId}/{chromaId}/{chromaId}.fantome
    const classicChromaPattern = /^classic\/(\d+)\/(\d+)\/(\d+)\/(\d+)\.(zip|fantome)$/
    const classicChromaMatch = relativePath.match(classicChromaPattern)

    if (classicChromaMatch) {
      const jadeChampionId = parseInt(classicChromaMatch[1], 10)
      const parentSkinId = classicChromaMatch[2]
      const chromaId = classicChromaMatch[3]
      const realChampionId = jadeChampionId - 60000
      ...
      if (champion) {
        ...
        const classicNum = parseInt(parentSkinId, 10) % 1000
        const classicSkin = champion.skins.find(
          (s) => s.skinType === 'classic' && s.num === classicNum + 10000
        )
        if (classicSkin) {
    ...
    // ID-based 4-level pattern (e.g., 236/236000/236003/236003.fantome)
    // This can be either a chroma OR a tiered/exalted skin variation
    const idBased4LevelPattern = /^(\d+)\/(\d+)\/(\d+)\/(\d+)\.(zip|fantome)$/
    const idBased4LevelMatch = relativePath.match(idBased4LevelPattern)

    if (idBased4LevelMatch) {
      const championId = parseInt(idBased4LevelMatch[1], 10)
      const parentSkinId = idBased4LevelMatch[2]
      const childId = idBased4LevelMatch[3]
      ...
      if (champion) {
        ...
        // If not a chroma, it's a tiered/exalted skin variation
        if (!isChroma) {
          const childNum = parseInt(childId, 10) - (championId * 1000)
    ...
    // Try ID-based regular skin pattern (e.g., 266/1/1.fantome or 266/1000/1000.zip)
    const idBasedPattern = /^(\d+)\/(\d+)\/(\d+)\.(zip|fantome)$/
    ...
    try {
      ...
      for (const championFolder of championFolders) {
        ...
        if (stat.isDirectory()) {
          ...
          // Resolve numeric champion folder to champion name
          let resolvedChampionName = decodedChampionName
          if (/^\d+$/.test(decodedChampionName)) {
            const savedLang = settingsService.get('language') || 'en_US'
            const champion = championDataService.getChampionByIdSync(parseInt(decodedChampionName, 10), savedLang)
            if (champion) {
              resolvedChampionName = champion.name
            }
          }
          ...
          for (const skinFile of skinFiles) {
            ...
            // Resolve numeric skin filename (e.g., "103086.fantome") to human-readable name
            let skinName = path.basename(skinFile)
            const numericSkinMatch = skinName.match(/^(\d+)\.(zip|fantome)$/)
            if (numericSkinMatch && /^\d+$/.test(decodedChampionName)) {
              const championId = parseInt(decodedChampionName, 10)
              const skinIdNum = parseInt(numericSkinMatch[1], 10)
              const skinNum = skinIdNum - (championId * 1000)
              const savedLang = settingsService.get('language') || 'en_US'
              const champion = championDataService.getChampionByIdSync(championId, savedLang)
              if (champion) {
                const skin = champion.skins.find((s) => s.num === skinNum)
                if (skin) {
  ...
  async downloadAllSkinsFromRepository(
    ...
    try {
      ...
      // Find the actual extracted folder (GitHub archives create a folder like "RepoName-branch")
      const extractedEntries = await fs.readdir(extractPath)
      console.log(`[SkinDownloader] Extracted entries: ${extractedEntries.join(', ')}`)

      const repoFolder = extractedEntries.find((entry) =>
        entry.toLowerCase().startsWith(repository.repo.toLowerCase())
      ...
      if (!existsSync(skinsPath)) {
        console.error(`[SkinDownloader] Skins path does not exist: ${skinsPath}`)
        console.error(`[SkinDownloader] Extract path contents: ${extractedEntries.join(', ')}`)
        // Try to find skins folder recursively
        const repoFolderPath = path.join(extractPath, repoFolder)
        if (existsSync(repoFolderPath)) {
          const repoContents = await fs.readdir(repoFolderPath)
          console.error(`[SkinDownloader] Repo folder contents: ${repoContents.join(', ')}`)
        }
        throw new Error(`Skins path not found in extracted archive: ${skinsPath}`)
      }

      console.log(`[SkinDownloader] Skins path resolved to: ${skinsPath}`)
  ...
  private async processSkins(
    ...
    for (const championDir of championDirs) {
      ...
      // Resolve champion name: if directory is numeric (ID-based), look up the name
      let resolvedChampionName = championDir
      const isIdBased = /^\d+$/.test(championDir)
      if (isIdBased) {
        const champion = championDataService.getChampionByIdSync(parseInt(championDir, 10), savedLanguage)
        if (champion) {
          resolvedChampionName = champion.name
        } else {
          console.warn(`[BulkDownload] Could not resolve champion ID: ${championDir}, skipping`)
          continue
        }
      }
      ...
      for (const skinEntry of skinEntries) {
        ...
        if (skinStat.isDirectory()) {
                ...
                for (const chromaFile of chromaFiles) {
                  if (/\.(zip|fantome)$/i.test(chromaFile)) {
                    files.push({
                      ...
                      destination: path.join(this.cacheDir, resolvedChampionName, chromaFile),
          ...
          // Check if this is an ID-based skin directory (numeric skinId like "11009")
          if (isIdBased && /^\d+$/.test(skinEntry)) {
            const championId = parseInt(championDir, 10)
            const skinIdNum = parseInt(skinEntry, 10)
            const skinNum = skinIdNum - (championId * 1000)

            const champion = championDataService.getChampionByIdSync(championId, savedLanguage)
            const skin = champion?.skins.find((s) => s.num === skinNum)
            const baseSkinName = skin ? (skin.nameEn || skin.name).replace(/[:/\\*?"<>|]/g, '') : skinEntry
            ...
            for (const inner of innerEntries) {
              ...
              if (innerStat.isDirectory() && !options.excludeChromas) {
                // Check if this subdirectory is itself a known skin (tiered/variant)
                const innerIdNum = parseInt(inner, 10)
                const innerSkinNum = innerIdNum - (championId * 1000)
                const innerSkin = champion?.skins.find((s) => s.num === innerSkinNum)

                if (innerSkin) {
                  // This is a tiered skin variant (e.g., 103086 = Immortalized Legend Ahri)
                  // Process it as its own skin with its own name
                  const innerBaseName = (innerSkin.nameEn || innerSkin.name).replace(/[:/\\*?"<>|]/g, '')
                  const innerFiles = await fs.readdir(innerPath)
                  for (const innerFile of innerFiles) {
                    const innerFilePath = path.join(innerPath, innerFile)
                    const innerFileStat = await fs.stat(innerFilePath)

                    if (innerFileStat.isDirectory()) {
                      // Sub-variant directory (e.g., 103087 under 103085 via 103086-adjacent)
                  ...
                  // Check if this ID is a chroma/variant of a sibling tiered skin
                  let chromaParentName = baseSkinName
                  if (champion) {
                    for (const s of champion.skins) {
                      if (s.chromaList?.some((c) => c.id.toString() === inner)) {
                        chromaParentName = (s.nameEn || s.name).replace(/[:/\\*?"<>|]/g, '')
                        break
                      }
                    }
                  }

                  // Regular chroma subdirectory in ID-based structure
          ...
          for (const inner of innerEntries) {
            ...
            if (innerStat.isDirectory() && !options.excludeChromas) {
              ...
              // Skin zip/fantome file inside the skin directory
              files.push({
                source: innerPath,
                destination: path.join(this.cacheDir, resolvedChampionName, inner),
                championName: resolvedChampionName,
                skinName: inner
              })
            }
          }
        } else if (/\.(zip|fantome)$/i.test(skinEntry)) {
          // Flat name-based: regular skin file directly in champion dir
          files.push({
            source: skinEntryPath,
            destination: path.join(this.cacheDir, resolvedChampionName, skinEntry),
            championName: resolvedChampionName,
            skinName: skinEntry
          })
        }
