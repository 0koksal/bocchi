...
export class ModToolsWrapper {
    ...
    try {
      ...
      for (let index = 0; index < validSkinMods.length; index++) {
        ...
        if (existingFolders && existingFolders.length > 0) {
          ...
          // Check if pre-imported
          const preImported = await preImportService.findPreImported(baseName)
          if (preImported) {
            // Pre-imported! Just rename it
            const tempName = `temp_${Date.now()}_${index}_${baseName}`
            renameOperations.push({
              from: preImported,
              to: targetModName,
            ...
            console.info(`[ModToolsWrapper] Using pre-imported: ${preImported} -> ${targetModName}`)
          } else {
            // Need to import from scratch
      ...
      // Use mod-tools.exe mkoverlay (reliable, correct WAD format)
      const modToolsPath = this.getModToolsExePath()
      if (!modToolsPath) {
        throw new Error('Mod tools path not found')
      }
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        try {
          ...
          console.debug(`[ModToolsWrapper] Executing mkoverlay (Attempt ${attempt}): ${mkoverlayArgs.join(' ')}`)
          ...
          console.error(`[ModToolsWrapper] Overlay creation attempt ${attempt} failed:`, error)
        }
      }

      if (!overlaySuccess) {
        throw new Error('Failed to create overlay after 3 attempts')
      }

      await new Promise((resolve) => setTimeout(resolve, 200))

      // Check for cancellation before starting injection
      if (this.isCancelled) {
        throw new Error('Operation cancelled by user')
      }
  ...
  async stopOverlay(): Promise<void> {
    // Stop native injector first
    stopInjection()

    // Also stop any legacy mod-tools process
  ...
  isRunning(): boolean {
    return isInjectionRunning() || (this.runningProcess !== null && !this.runningProcess.killed)
  }
