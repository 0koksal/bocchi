...
export class ToolsDownloader {
  ...
  async checkToolsExist(): Promise<boolean> {
    try {
      ...
      await fs.promises.access(ltkHostPath, fs.constants.F_OK)
      await fs.promises.access(ltkDllPath, fs.constants.F_OK)
      return true
    } catch {
      return false
    }
  ...
  async downloadAndExtractTools(
    ...
    try {
      ...
      try {
        console.log('[ToolsDownloader] Downloading LTK Patcher binaries...')
        const ltkBaseUrl = 'https://raw.githubusercontent.com/LeagueToolkit/ltk-manager/main/src-tauri/resources'

        const ltkHostPath = path.join(targetPath, 'ltk_patcher_host.exe')
        const ltkDllPath = path.join(targetPath, 'ltk_patcher_dll.dll')

        // Download host exe
        const hostResponse = await axios.get(`${ltkBaseUrl}/ltk_patcher_host.exe`, {
          responseType: 'arraybuffer',
          timeout: 60000
        })
        await fs.promises.writeFile(ltkHostPath, Buffer.from(hostResponse.data))
        console.log('[ToolsDownloader] Downloaded ltk_patcher_host.exe')

        // Download DLL
        const dllResponse = await axios.get(`${ltkBaseUrl}/ltk_patcher_dll.dll`, {
          responseType: 'arraybuffer',
          timeout: 60000
        })
