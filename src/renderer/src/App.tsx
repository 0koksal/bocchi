  ...
  const { onChampionNavigate, clearSelectedChampion } = useChampionSelectHandler({
    ...
    onAutoSelectSkin: async (champion) => {
      ...
      if (!isCustomSkinSelected && randomSkin) {
        ...
        // For classic skins, pass the skin ID so the URL constructor can extract the classic num
        const classicSkinFile = randomSkin.skinType === 'classic' ? randomSkin.id : skinFileName
        const urlResult = await window.api.repositoryConstructUrl(
          ...
          randomSkin.skinType === 'classic' // Pass isClassic for classic skins
        )
    ...
    (champion: Champion, skin: Skin, chromaId?: string, variantId?: string) => {
      ...
      if (existingIndex >= 0) {
        ...
        try {
          ...
          // Non-critical, import will happen at Apply time if pre-import fails
        }
      }
    },
    [gamePath, setStatusMessage, t, selectedSkins, setSelectedSkins, downloadedSkins]
  )
