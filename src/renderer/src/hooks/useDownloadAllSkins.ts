...
export function useDownloadAllSkins() {
  ...
  const startDownloadWithOptions = useCallback(async (options: DownloadOptions) => {
    ...
    try {
      console.log('[DownloadAll] Starting bulk download with options:', options)
      ...
      console.log('[DownloadAll] Bulk download result:', result)
      if (!result.success) {
        console.error('[DownloadAll] Failed:', result.error)
