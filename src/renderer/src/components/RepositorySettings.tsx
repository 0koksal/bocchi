...
export function RepositorySettings({ disabled }: RepositorySettingsProps) {
  ...
  const handleRedetect = async (repositoryId: string) => {
    try {
      ...
      if (result.success && result.data) {
        ...
        toast.success(
          `Structure re-detected: ${structureType} (${result.data.confidence}% confidence)`,
          { duration: 5000 }
        )
        await loadRepositories()
      } else {
  ...
  const handleRemove = async (repositoryId: string) => {
    ...
    if (
      !confirm(t('settings.repositories.confirmRemove', 'Are you sure you want to remove this repository?'))
    ) {
      return
    }
  ...
  const parseGitHubUrl = (url: string) => {
    try {
      ...
      if (match) {
        ...
        return { owner, repo, branch, name: `${owner}/${repo}` }
      }
    } catch (error) {
  ...
  const handleAddRepository = async () => {
    if (!newRepo.name || !newRepo.owner || !newRepo.repo) {
      toast.error(t('settings.repositories.fillAllFields', 'Please fill all required fields'))
      return
    }
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        setRepositories([...repositories, repository as Repository])
        ...
        toast.success(
          `Repository added! Structure: ${structureType} (${detection.confidence}% confidence)`,
          { duration: 5000 }
        )
  ...
  const getStatusIcon = (status?: string) => {
    switch (status) {
      ...
      case 'checking':
        return (
          <div className="flex items-center gap-1" title="Checking">
      ...
      default:
        return (
          <div className="flex items-center gap-1" title="Unchecked">
            <AlertCircle className="w-4 h-4 text-yellow-500" />
            <span className="text-xs text-yellow-500">Unchecked</span>
          </div>
        )
    }
  }
        ...
        <div>
          ...
          <p className="text-xs text-text-secondary mt-1">
            {t(
              ...
              'Manage skin source repositories. Add custom GitHub repositories with League of Legends skins.'
            )}
          </p>
        </div>
        <Button
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {t(
                ...
                'Add a GitHub repository containing League of Legends skin files.'
              )}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="repo-name">
                {t('settings.repositories.addDialog.name', 'Name')}
              </Label>
              <Input
                id="repo-name"
                value={newRepo.name}
