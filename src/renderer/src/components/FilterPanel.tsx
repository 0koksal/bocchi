...
export const FilterPanel: React.FC<FilterPanelProps> = ({
  ...
  // Pre-import status
  const [preImportStatus, setPreImportStatus] = useState<{
    total: number
    completed: number
  ...
  useEffect(() => {
    const unsub = window.api.onPreImportStatus((status) => {
      if (status.inProgress || status.completed < status.total) {
        setPreImportStatus(status)
      } else {
        setPreImportStatus(null)
      }
    })
    return () => unsub()
  }, [])
      ...
      <div className="px-8 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 text-sm text-text-secondary flex-wrap">
            ...
            {preImportStatus && (
              <span className="whitespace-nowrap text-xs text-primary-500 animate-pulse">
                ⚡ Pre-importing {preImportStatus.completed}/{preImportStatus.total}
                {preImportStatus.inProgress ? `: ${preImportStatus.inProgress}` : ''}
              </span>
            )}
            <span className="whitespace-nowrap">
              {resultsCount === 1
