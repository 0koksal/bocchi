...
import { auxWindowEnabledAtom } from '../store/atoms/settings.atoms'
...
export function TitleBar({ appVersion }: TitleBarProps) {
  ...
  const auxWindowEnabled = useAtomValue(auxWindowEnabledAtom)
  ...
  return (
    <div className="fixed top-0 left-0 right-0 h-10 bg-surface border-b-2 border-border flex items-center justify-between select-none z-50 shadow-md dark:shadow-none">
        ...
        <div className="flex items-center gap-1 ml-2">
          ...
          {auxWindowEnabled && (
            <Button
              ...
              size="icon"
              className="w-8 h-8 rounded"
              onClick={() => window.api.toggleAuxWindow()}
              aria-label="Mini Window"
              title="Mini Window (Dodge / Lobby Reveal)"
            >
              <svg
      ...
      <DiscordDialog open={showDiscordDialog} onOpenChange={setShowDiscordDialog} />
      <GithubDialog open={showGithubDialog} onOpenChange={setShowGithubDialog} />
    </div>
  )
}
