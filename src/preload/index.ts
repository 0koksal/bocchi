...
const api = {
  ...
  // Pre-import status
  getPreImportStatus: () => ipcRenderer.invoke('preimport:get-status'),
  preImportSkin: (skinPath: string, championName: string) =>
    ipcRenderer.invoke('preimport:queue', skinPath, championName),
  onPreImportStatus: (callback: (status: { total: number; completed: number; inProgress: string | null; failures: string[] }) => void) => {
    const handler = (_: IpcRendererEvent, status: any) => callback(status)
    ipcRenderer.on('preimport:status', handler)
    return () => ipcRenderer.removeListener('preimport:status', handler)
  }
}
